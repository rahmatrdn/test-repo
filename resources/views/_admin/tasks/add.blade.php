...
@section('content')
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
            ...
            <form navigate-form action="{{ route('teacher.tasks.do_create') }}" method="POST" class="p-6">
                @csrf
                <div class="space-y-4">
                    <!-- Form Group -->
                    <div>
                        ...
                        <div class="relative">
                            <select name="task_category_id" id="task_category_id"
                                ...
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}"
                                        {{ old('task_category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="w-1/2">
                            <label for="task_date" class="block text-sm font-medium mb-2 dark:text-white">Tanggal <span
                                    class="text-red-500">*</span></label>
                            <div class="relative">
                                <input type="text" id="task_date" name="task_date" value="{{ old('task_date') }}"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <textarea id="description" name="description" rows="3"
                                class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600 @error('description') border-red-500 focus:border-red-500 focus:ring-red-500 @enderror">{{ old('description') }}</textarea>
                        </div>
                        @error('description')
